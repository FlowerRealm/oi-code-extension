...
exports.OI_CODE_TEST_TMP_PATH = exports.OI_CODE_TEST_BASE_PATH = exports.OI_CODE_TEST_DIR = void 0;
const os = __importStar(__webpack_require__(/*! os */ "os"));
const path = __importStar(__webpack_require__(/*! path */ "path"));
/**
 * Shared constants for the OI-Code extension
 */
exports.OI_CODE_TEST_DIR = '.oi-code-tests';
exports.OI_CODE_TEST_BASE_PATH = path.join(os.homedir(), exports.OI_CODE_TEST_DIR);
exports.OI_CODE_TEST_TMP_PATH = path.join(exports.OI_CODE_TEST_BASE_PATH, 'tmp');
...
class Installer {
    ...
    static waitForDockerReady() {
        return __awaiter(this, arguments, void 0, function* (timeoutMs = 180000) {
     ...
     */
    static ensureDockerAvailableSilently() {
            ...
            yield vscode.window.withProgress({
                ...
                function run(cmd, args, cwd) {
                    return new Promise((resolve) => {
                        ...
                        child.on('close', code => resolve(code !== null && code !== void 0 ? code : 0));
                    });
                }
                try {
                    if (platform === 'win32') {
                        ...
                        if (this.isCommandAvailable('winget')) {
                            ...
                            yield run('winget', ['install', '-e', '--id', 'Docker.DockerDesktop', '--silent', '--accept-package-agreements', '--accept-source-agreements']);
                        }
                        else if (this.isCommandAvailable('choco')) {
                            progress.report({ message: 'Installing Docker Desktop via choco (silently)...' });
                            yield run('choco', ['install', 'docker-desktop', '-y', '--no-progress']);
                        }
                        ...
                        if (this.isCommandAvailable('brew')) {
                            ...
                            yield run('brew', ['install', '--cask', 'docker']);
                        }
                        try {
                        ...
                        try {
                            if (distro === 'ubuntu' || distro === 'debian') {
                                yield run('bash', ['-lc', 'sudo apt-get update && sudo apt-get install -y docker.io']);
                            }
                            else if (distro === 'arch') {
                                yield run('bash', ['-lc', 'sudo pacman -Syu --noconfirm docker']);
                            }
                            else if (distro === 'fedora') {
                                yield run('bash', ['-lc', 'sudo dnf install -y dnf-plugins-core && sudo dnf config-manager --add-repo https://download.docker.com/linux/fedora/docker-ce.repo && sudo dnf install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin']);
                            }
                        }
                        catch (_c) { }
                        // Try starting docker service (if applicable)
                        try {
                            yield run('bash', ['-lc', 'sudo systemctl start docker']);
                        }
                        catch (_d) { }
                    }
                    progress.report({ message: 'Waiting for Docker to be ready...' });
                    yield this.waitForDockerReady();
                    output.appendLine('Docker is ready.');
                }
                catch (e) {
                    output.appendLine(`Silent installation failed: ${(e === null || e === void 0 ? void 0 : e.message) || e}`);
                    // 向上层传递错误，让调用者决定如何处理
                    throw e;
                }
            }));
...
const path = __importStar(__webpack_require__(/*! path */ "path"));
const vscode = __importStar(__webpack_require__(/*! vscode */ "vscode"));
const install_1 = __webpack_require__(/*! ./docker/install */ "./src/docker/install.ts");
...
class DockerManager {
    ...
    static ensureImage(projectRootPath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield install_1.Installer.ensureDockerAvailableSilently();
            }
            catch (error) {
                console.log('Silent Docker installation failed, proceeding with manual flow');
            }
            // Verify Docker is working
            const isDockerWorking = yield this.checkDockerWorking();
            if (!isDockerWorking) {
                throw new Error('Docker is not available or not working properly');
            }
        });
    }
    static checkDockerWorking() {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve) => {
                const check = (0, child_process_1.spawn)('docker', ['info'], { stdio: 'ignore' });
                check.on('close', (code) => {
                    resolve(code === 0);
                });
                check.on('error', () => {
    ...
    static run(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield fs.mkdir(constants_1.OI_CODE_TEST_TMP_PATH, { recursive: true });
            const tempDir = yield fs.mkdtemp(path.join(constants_1.OI_CODE_TEST_TMP_PATH, 'oi-run-'));
            ...
            return new Promise((resolve) => {
                ...
                const killTimer = setTimeout(() => {
                    ...
                    (0, child_process_1.spawn)('docker', ['kill', containerName]).on('close', () => { });
                    timedOut = true;
                }, (timeLimit + 1) * 1000);
    ...
    static selectImageForCommand(languageId) {
...
const fs = __importStar(__webpack_require__(/*! fs */ "fs"));
const dockerManager_1 = __webpack_require__(/*! ./dockerManager */ "./src/dockerManager.ts");
...
function postWebviewMessage(panel, command, data = {}) {
    try {
        panel.webview.postMessage(Object.assign({ command }, data));
    }
    catch (e) {
...
function getPairCheckWebviewContent(webview) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function runSingleInDocker(projectRootPath, sourceDir, languageId, sourceFileName, input, options) {
    return __awaiter(this, void 0, void 0, function* () {
        // Prefer configured docker compilers; fallback to sane defaults
        const config = vscode.workspace.getConfiguration();
        const compilers = config.get('oicode.docker.compilers') || {};
        const defaultOpt = config.get('oicode.compile.opt');
        const defaultStd = config.get('oicode.compile.std');
        // Helper function to apply compiler options
        function applyCompilerOptions(args, options) {
            let result = [...args];
            const effOpt = (options === null || options === void 0 ? void 0 : options.opt) || defaultOpt;
            if (effOpt) {
                result = result.map(a => /^-O[0-3]$/.test(a) ? `-${effOpt}` : a);
                if (!result.some(a => /^-O[0-3]$/.test(a)))
                    result.push(`-${effOpt}`);
            }
            const effStd = (options === null || options === void 0 ? void 0 : options.std) || defaultStd;
            if (effStd) {
                result = result.map(a => a.startsWith('-std=') ? `-std=${effStd}` : a);
                if (!result.some(a => a.startsWith('-std=')))
                    result.push(`-std=${effStd}`);
            }
            return result;
        }
        let command;
        if (languageId === 'python') {
            ...
            const args = py.args.map(a => a.replace(/\$\{sourceFile\}/g, sourceFileName));
            command = [py.command, ...args].join(' ');
        }
        else if (languageId === 'cpp') {
            const cpp = compilers.cpp || { command: 'g++', args: ['/sandbox/${sourceFile}', '-o', '/tmp/a.out', '-O2', '-std=c++17'] };
            let args = cpp.args.map(a => a.replace(/\$\{sourceFile\}/g, sourceFileName));
            args = applyCompilerOptions(args, options);
            command = [cpp.command, ...args, '&&', '/tmp/a.out'].join(' ');
        }
        else {
            const c = compilers.c || { command: 'gcc', args: ['/sandbox/${sourceFile}', '-o', '/tmp/a.out', '-O2'] };
            let args = c.args.map(a => a.replace(/\$\{sourceFile\}/g, sourceFileName));
            args = applyCompilerOptions(args, options);
            command = [c.command, ...args, '&&', '/tmp/a.out'].join(' ');
        }
        const result = yield dockerManager_1.DockerManager.run({
            sourceDir,
            command,
            input,
            memoryLimit: '512',
...
function createDiffHtml(output1, output2) {
        ...
        webviewView.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            if (message.command === 'runPairCheck') {
                try {
                    ...
                    yield fs.promises.mkdir(constants_1.OI_CODE_TEST_BASE_PATH, { recursive: true });
                    const tempDir = yield fs.promises.mkdtemp(path.join(constants_1.OI_CODE_TEST_BASE_PATH, 'pair-'));
                    const file1Path = path.join(tempDir, `code1.${langId}`);
                    const file2Path = path.join(tempDir, `code2.${langId}`);
                    yield fs.promises.writeFile(file1Path, editor1.document.getText());
                    yield fs.promises.writeFile(file2Path, editor2.document.getText());
                    const ext = langId === 'python' ? 'py' : langId;
                    const [result1, result2] = yield Promise.all([
                        runSingleInDocker(this._context.extensionPath, tempDir, langId, `code1.${ext}`, message.input).catch((e) => ({ stdout: '', stderr: e.message })),
                        runSingleInDocker(this._context.extensionPath, tempDir, langId, `code2.${ext}`, message.input).catch((e) => ({ stdout: '', stderr: e.message }))
                    ]);
...
PairCheckViewProvider.viewType = 'oicode.pairCheckView';
  ...
  <script>
    ...
    document.getElementById('loadSamples').addEventListener('click', ()=>{
      vscode.postMessage({ cmd:'loadSamples' });
                ...
                function pickProblemsBaseDir() {
                    return __awaiter(this, void 0, void 0, function* () {
                        const saved = context.globalState.get('oicode.lastProblemsBaseDir');
                        if (saved) {
                            try {
                                yield fs.promises.access(saved);
                                const choice = yield vscode.window.showQuickPick([
                                    { label: `使用上次：${saved}`, value: 'saved' },
                                    { label: '重新选择...', value: 'pick' }
                                ], { placeHolder: '选择题目根目录' });
                                if (!choice) {
                                    throw new Error('未选择题目根目录');
                                }
                        ...
                        const pick = yield vscode.window.showOpenDialog({ canSelectFolders: true, canSelectFiles: false, canSelectMany: false, openLabel: '选择题目根目录' });
                        if (!pick || !pick[0]) {
                            throw new Error('未选择题目根目录');
                        }
                ...
                function ensureProblemStructure(m) {
                    return __awaiter(this, void 0, void 0, function* () {
                        ...
                        yield fs.promises.writeFile(path.join(configDir, 'problem.json'), JSON.stringify(configJson, null, 2), 'utf8');
                        if (m.statement)
                            yield fs.promises.writeFile(path.join(configDir, 'statement.md'), m.statement, 'utf8');
                        if (m.samples)
                            yield fs.promises.writeFile(path.join(configDir, 'samples.txt'), m.samples, 'utf8');
                        return { sourcePath };
                    });
                }
                webviewView.webview.onDidReceiveMessage((m) => __awaiter(this, void 0, void 0, function* () {
                    if (m.cmd === 'loadSamples') {
                        const uris = yield vscode.window.showOpenDialog({ canSelectMany: false, openLabel: '选择样例文件' });
                        if (uris && uris[0]) {
                            const buf = yield vscode.workspace.fs.readFile(uris[0]);
                        ...
                        try {
                            const { sourcePath } = yield ensureProblemStructure(m);
                            const doc = yield vscode.workspace.openTextDocument(vscode.Uri.file(sourcePath));
                            yield vscode.window.showTextDocument(doc, { preview: false });
                            yield vscode.commands.executeCommand('oicode.runCode', m.samples || '');
                        }
                        catch (e) {
                        ...
                        yield vscode.commands.executeCommand('oicode.runPairCheck', m.samples || '');
                    }
                }));
            }
        }));
        // Command: create a new problem skeleton by name
        context.subscriptions.push(vscode.commands.registerCommand('oicode.createProblem', (payload) => __awaiter(this, void 0, void 0, function* () {
            try {
                let name = payload === null || payload === void 0 ? void 0 : payload.name;
                if (!name) {
                    name = (yield vscode.window.showInputBox({ prompt: '输入题目名称（将作为文件夹名）', placeHolder: '如：CF1234A' })) || '';
                }
                if (!name) {
                    return;
                }
                const safe = name.replace(/[^\w\-\.]+/g, '_').slice(0, 64);
                let baseDir = (payload === null || payload === void 0 ? void 0 : payload.baseDir) || context.globalState.get('oicode.lastProblemsBaseDir');
                if (baseDir) {
                    try {
                        yield fs.promises.access(baseDir);
                    }
                    catch (_a) {
                        baseDir = undefined;
                    }
                ...
                context.globalState.update('oicode.lastProblemsBaseDir', baseDir);
                let langId = payload === null || payload === void 0 ? void 0 : payload.language;
                if (!langId) {
                    const langPick = yield vscode.window.showQuickPick([
                    ...
                    try {
                        ...
                        const tpl = langId === 'c'
                            ...
                            : langId === 'cpp'
                                ...
                                : 'print("TODO")\n';
                        yield fs.promises.writeFile(sourcePath, tpl, 'utf8');
                    }
                    // Default config
                    const problemJsonPath = path.join(configDir, 'problem.json');
                    try {
                        yield fs.promises.access(problemJsonPath);
                    }
                    catch (_c) {
                        yield fs.promises.writeFile(problemJsonPath, JSON.stringify({ name: safe, url: '', timeLimit: 5, memoryLimit: 256, opt: '', std: '' }, null, 2), 'utf8');
                    }
                    const statementPath = path.join(configDir, 'statement.md');
                    try {
                        yield fs.promises.access(statementPath);
                    }
                    catch (_d) {
                        yield fs.promises.writeFile(statementPath, `# ${safe}\n\n在此编写题面...\n`, 'utf8');
                    }
                    const samplesPath = path.join(configDir, 'samples.txt');
                    try {
                        yield fs.promises.access(samplesPath);
                    }
                    catch (_e) {
                        yield fs.promises.writeFile(samplesPath, '', 'utf8');
                    }
                    const doc = yield vscode.workspace.openTextDocument(vscode.Uri.file(sourcePath));
                    yield vscode.window.showTextDocument(doc, { preview: false });
        ...
        context.subscriptions.push(vscode.window.registerWebviewViewProvider(PairCheckViewProvider.viewType, pairCheckProvider));
        context.subscriptions.push(vscode.commands.registerCommand('oi-code.showSettingsPage', () => {
            const panel = vscode.window.createWebviewPanel('oiCodeSettings', 'OI-Code 设置', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        ...
        context.subscriptions.push(vscode.commands.registerCommand('oicode.startPairCheck', () => __awaiter(this, void 0, void 0, function* () {
            ...
            const pairDoc = yield vscode.workspace.openTextDocument({
                content: `// 在这里粘贴或编写你的对拍代码\n// 例如，一个暴力解法\n`,
                language: originalDoc.languageId
            });
            yield vscode.window.showTextDocument(pairDoc, vscode.ViewColumn.Beside);
            yield vscode.commands.executeCommand('oicode.pairCheckView.focus');
        })));
        // Programmatic pair-check command for tests and headless execution
        context.subscriptions.push(vscode.commands.registerCommand('oicode.runPairCheck', (testInput) => __awaiter(this, void 0, void 0, function* () {
            ...
            yield fs.promises.mkdir(constants_1.OI_CODE_TEST_TMP_PATH, { recursive: true });
            const tempDir = yield fs.promises.mkdtemp(path.join(constants_1.OI_CODE_TEST_TMP_PATH, 'oi-code-'));
            try {
                const ext = langId === 'python' ? 'py' : langId;
                ...
                yield fs.promises.writeFile(file1Path, editor1.document.getText());
                yield fs.promises.writeFile(file2Path, editor2.document.getText());
                const input = testInput !== null && testInput !== void 0 ? testInput : '';
                const [result1, result2] = yield Promise.all([
                    runSingleInDocker(context.extensionPath, tempDir, langId, `code1.${ext}`, input).catch((e) => ({ stdout: '', stderr: e.message })),
                    runSingleInDocker(context.extensionPath, tempDir, langId, `code2.${ext}`, input).catch((e) => ({ stdout: '', stderr: e.message }))
                ]);
                const output1 = result1.stderr ? `ERROR:\n${result1.stderr}` : result1.stdout;
                const output2 = result2.stderr ? `ERROR:\n${result2.stderr}` : result2.stdout;
        ...
        context.subscriptions.push(vscode.commands.registerCommand('oicode.runCode', (testInput) => __awaiter(this, void 0, void 0, function* () {
            ...
            return vscode.window.withProgress({
                ...
                try {
                    const result = yield runSingleInDocker((((_a = vscode.extensions.getExtension('oi-code')) === null || _a === void 0 ? void 0 : _a.extensionPath) || ''), path.dirname(document.uri.fsPath), languageId, sourceFile, input || '', { timeLimit, memoryLimit });
                    progress.report({ increment: 100 });
                    const panel = vscode.window.createWebviewPanel('oiCodeOutput', `Output for ${sourceFile}`, vscode.ViewColumn.Two, {});
                    const meta = [];
                    if (result.timedOut)
                        meta.push('TimedOut');
                    if (result.memoryExceeded)
                        meta.push('MemoryExceeded');
                    if (result.spaceExceeded)
                        meta.push('SpaceExceeded');
                    let content = '';
                    if (meta.length)
                        content += `<p><b>Flags:</b> ${meta.join(', ')}</p>`;
                    if (result.stdout)
                        content += `<h2>Output:</h2><pre>${htmlEscape(result.stdout)}</pre>`;
                    if (result.stderr)
                        content += `<h2>Error:</h2><pre>${htmlEscape(result.stderr)}</pre>`;
        ...
        context.subscriptions.push(vscode.commands.registerCommand('oicode.initializeEnvironment', () => __awaiter(this, void 0, void 0, function* () {
            vscode.window.withProgress({
                ...
                try {
                    yield dockerManager_1.DockerManager.ensureImage(context.extensionPath);
        ...
        context.subscriptions.push(vscode.commands.registerCommand('oicode.downloadDocker', () => __awaiter(this, void 0, void 0, function* () {
            const installCommand = install_1.Installer.getInstallCommand();
            if (installCommand) {
                vscode.window.showInformationMessage(installCommand.message, 'Run in Terminal' // Button text
                ).then(selection => {
...
function getWebviewContent(context, fileName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const content = yield vscode.workspace.fs.readFile(filePath);
